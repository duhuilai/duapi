...
### v1.7.4 (2026-06-29)
- ✨ **搜索定位增强** — 输入搜索关键词后自动展开匹配的分组，首个匹配接口以橙色高亮定位并自动滚动到可见区域
- 🔧 **移除环境切换** — 侧边栏移除环境切换下拉框，简化界面
- ✨ **表格列宽拖拽** — 所有 Schema 表格（响应参数、Body 参数、错误码）的列宽可通过拖拽列边框线自由调整

### v1.7.3 (2026-06-29)
- ✨ **响应复制按钮** — 响应面板标题栏新增「📋 复制」按钮，一键复制完整响应内容到剪贴板，复制后显示「✓ 已复制」反馈
