...
- 🎨 **导航按钮优化** — 分段控件（Segmented Control）风格，激活态渐变背景 + 发光阴影 + 图标，视觉效果更醒目

### v0.0.10 (2026-07-01)

- 🎨 **更换应用图标** — 重新替换 logo，自动裁剪多余白边，图标主体更饱满
- ✨ **切换接口保留响应结果** — 每个接口独立保存上次响应，切换接口时自动恢复，无需重新发送请求

### v0.0.9 (2026-07-01)

- 🐛 **修复切换接口时响应 body 不更新** — 切换接口后响应面板正确显示为空状态，不再残留上一个接口的响应数据

### v0.0.8 (2026-07-01)

- ✨ **分组内接口拖拽排序** — 支持在侧边栏拖拽接口调整顺序，拖拽时显示蓝色指示线，便于按需排列接口生成文档

### v0.0.7 (2026-06-30)

- 🐛 **修复文档参数层级丢失** — Schema 参数表格中嵌套字段现在显示完整路径（如 `list[].status`），不再只显示最后一段
- ✨ **响应示例始终输出** — 响应示例（JSON）代码块始终出现在文档中，无数据时显示 `{}`

### v0.0.6 (2026-06-29)

- ✨ **文档格式优化** — 严格按新版模板调整文档生成格式
  - 📋 **新增文档元数据** — 自动生成标题/作者/日期，标题使用首个分组名
  - 🔢 **全局接口编号** — 接口按全局序号排列（1、2、3…），不再按分组独立编号
  - 🏷️ **标签扁平化** — 移除各字段标签的 H2 标题前缀，改用纯文本标签
  - 📦 **响应示例简化** — 合并成功/失败响应编码为单一编码块
